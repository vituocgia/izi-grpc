...
from izi_grpc.utils import import_string
from izi_grpc import create_app
...
def _load_jobs():
    ...
    # load builtin
    import_string('izi_grpc.cmds')

    # load lib jobs
    for ep in pkg_resources.iter_entry_points('izi_grpc.jobs'):
        try:
            ep.load()
        except Exception as e:
            logger = logging.getLogger('izi_grpc.cmd')
            logger.debug(
...
def _run(root):
    # show help message when user run izi_grpc command directly
    args = sys.argv[1:] or ['--help']
    known, argv = root.parse_known_args(args)
    kwargs = vars(known)
    handler = kwargs.pop('handler')
    os.environ.setdefault('IZI_ENV', handler.env)
    if handler.inapp:
        create_app()
    try:
        if handler.proxy:
...
def main():
    root = argparse.ArgumentParser('izi_grpc')
